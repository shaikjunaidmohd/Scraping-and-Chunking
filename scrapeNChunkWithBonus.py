...
def prettify_with_llm(chunk):
    prompt = (
        "The text output of web scraping is often very messy. "
        "Please clean up, format and prettify the following information, ensuring that headers, paragraphs, lists, and tables are properly organized and readable:\n\n"
    )

    genai.configure(api_key=os.environ['GOOGLE_API_KEY'])
    model = genai.GenerativeModel('gemini-pro')

    response = model.generate_content(f"{prompt} + {chunk}")

    if response._done and response._result and response._result.candidates and len(response._result.candidates[0].content.parts) > 0:
        cleaned_content = response._result.candidates[0].content.parts[0].text
    else:
        return chunk
...
print("-------------prettified format--------------")
print(prettify_with_llm(final_chunks))